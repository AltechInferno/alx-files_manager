...
const fileUtils = {
 ...
 /**
   ...
   * @file {object} file to obtain data
   */
  ...
  /**
   * Transform _id into id in a file doc
   */
  processFile(doc) {
   ...
   * query {obj} query to find doc to update
   */
   ...
   * FOLDER_PATH {string} path to save file on disk
   * return {obj} obj with error if present in file
   */
  async saveFile(userId, fileParams, FOLDER_PATH) {
    ...
    const res = await dbClient.filesCollection.insertOne(query);
    ...
    const newFile = { id: res.insertedId, ...file };

    return { error: null, newFile };
  },

  /**
   * gets list of file docs from db belonging
   * to a parent_id
  ...
  /**
   * checks if body is valid for creating file
   * @request {request_object} express request obj
